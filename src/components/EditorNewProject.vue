  ...
  >
    <form class="project-form" @submit.prevent="handleCreate">
      <div class="field-grid">
        <label class="field">
          <span
            >{{ t('project.name') }} <i>{{ t('general.required') }}</i></span
          >
          <input
            v-model="form.name"
      ...
      <fieldset class="template-fieldset">
        ...
        <div v-if="templatesLoading" class="template-state" role="status">
          {{ t('general.loading') }}
        </div>
        ...
        <div v-else class="template-state template-state--error">
          ...
          <UiButton type="button" variant="secondary" size="sm" @click="loadTemplates">{{
            t('translate.retry')
          }}</UiButton>
        </div>
      </fieldset>
      ...
      <label class="field">
        <span
          >{{ t('project.location') }} <i>{{ t('general.required') }}</i></span
        >
        <div class="location-row">
          ...
          />
          <UiButton data-test="browse-btn" type="button" variant="secondary" @click="browseLocation"
            ><FolderOpen :size="15" />{{ t('project.browse') }}</UiButton
          >
        </div>
      </label>
      ...
      <label class="git-row">
        ...
        <span
          ><strong>{{ t('project.initGit') }}</strong
          ><small>{{ t('project.initGitDescription') }}</small></span
        >
      </label>
    ...
    <template #footer>
      <UiButton variant="secondary" :disabled="creating" @click="$emit('close')">{{
        t('general.cancel')
      }}</UiButton>
      <UiButton
...
const canCreate = computed(
  () => !!form.name.trim() && !!form.location && !creating.value && !templatesLoading.value,
)
...
async function loadTemplates() {
  ...
  try {
    ...
    if (
      ...
      !templates.value.some((template) => template.id === form.templateId)
    )
...
watch(
  () => props.visible,
  (visible) => {
    if (visible && !templates.value.length) void loadTemplates()
  },
)
...
.template-check {
  ...
  border-radius: 50%;
  background: var(--c-panel);
  box-shadow: inset 0 0 0 3px var(--c-panel);
}
