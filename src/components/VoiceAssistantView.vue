        ...
        <header class="va-header">
          <div class="va-title-row">
            <div class="va-orb-wrapper" aria-hidden="true">
              ...
              <span class="va-orb" :class="{ 'va-orb--active': state === 'listening' }"><Mic class="va-orb-icon" /></span>
            </div>
            <div><p>{{ t('voice.title') }}</p><strong class="va-status">{{ statusText }}</strong></div>
          </div>
          <button type="button" :aria-label="t('general.close')" @click="cancel"><X :size="17" /></button>
        </header>

        <div class="va-body">
          <div v-if="transcript" class="va-transcript" :data-label="t('voice.transcript')"><p>{{ transcript }}</p></div>
          <div v-else-if="state === 'listening'" class="va-listening-note">{{ t('voice.speakNow') }}</div>

          <Transition name="va-text">
            <div v-if="response" class="va-response"><span>{{ t('voice.result') }}</span><p>{{ response }}</p></div>
          </Transition>
          ...
          <Transition name="va-text">
            <div v-if="commandFeedback" class="va-cmd-feedback" :class="commandFeedback.success ? 'va-cmd-ok' : 'va-cmd-err'">
              ...
              <div><strong>{{ commandFeedback.text }}</strong><p v-if="commandFeedback.error" class="va-cmd-error-detail">{{ commandFeedback.error }}</p></div>
            </div>
          </Transition>
...
<style scoped>
.va-overlay { position: fixed; inset: 0; z-index: 2900; display: grid; place-items: center; padding: 24px; }
.va-backdrop { position: absolute; inset: 0; background: color-mix(in srgb, var(--c-text-0) 24%, transparent); backdrop-filter: blur(4px); cursor: pointer; }
.va-content { position: relative; z-index: 1; display: flex; width: min(500px, calc(100vw - 40px)); max-height: min(650px, calc(100vh - 48px)); flex-direction: column; overflow: hidden; border: 1px solid var(--c-border); border-radius: 12px; background: var(--c-panel); box-shadow: var(--elevation-4); color: var(--c-text-0); }
.va-header { display: flex; min-height: 76px; align-items: center; justify-content: space-between; gap: 20px; padding: 15px 18px; border-bottom: 1px solid var(--c-border); }
.va-title-row { display: flex; min-width: 0; align-items: center; gap: 13px; }.va-title-row > div:last-child { min-width: 0; }.va-title-row p { margin: 0 0 3px; color: var(--c-text-3); font-size: 11px; }.va-status { display: block; overflow: hidden; color: var(--c-text-0); font-family: var(--font-serif-zh); font-size: 16px; font-weight: 630; text-overflow: ellipsis; white-space: nowrap; }
.va-header button { display: grid; width: 30px; height: 30px; flex: 0 0 auto; place-items: center; border: 0; border-radius: 8px; background: transparent; color: var(--c-text-2); cursor: pointer; }.va-header button:hover { background: var(--c-surface-2); color: var(--c-text-0); }.va-header button:focus-visible { outline: none; box-shadow: var(--ring-focus); }
.va-orb-wrapper { position: relative; display: grid; width: 46px; height: 46px; flex: 0 0 auto; place-items: center; }.va-orb { display: grid; width: 40px; height: 40px; place-items: center; border: 1px solid var(--c-border); border-radius: 10px; background: var(--c-accent-soft); color: var(--c-accent); }.va-orb-icon { width: 19px; height: 19px; }.va-orb--active { border-color: color-mix(in srgb, var(--c-accent) 45%, var(--c-border)); }
.va-ripple { position: absolute; bottom: -1px; left: 50%; z-index: 2; width: 2px; height: 7px; border-radius: 2px; background: var(--c-accent); animation: va-level 900ms ease-in-out infinite alternate; }.va-ripple-1 { margin-left: -7px; animation-delay: -500ms; }.va-ripple-2 { height: 10px; margin-left: -1px; animation-delay: -250ms; }.va-ripple-3 { margin-left: 5px; }
...
@media (prefers-reduced-motion: reduce) { .va-ripple { animation: none; } }
</style>
