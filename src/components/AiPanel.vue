  ...
  <div class="ai-chat-panel" :class="{ 'workspace-variant': workspaceVariant }">
    ...
    <div class="ac-header">
      <div class="ac-title-row">
        ...
        <span class="ac-title">{{
          workspaceVariant ? t('aiPanel.writingTitle') : t('aiPanel.title')
        }}</span>
      </div>
      <div class="ac-header-actions">
        <button
          class="ac-icon-btn u-interactive"
          @click="clearHistory"
          :title="t('aiPanel.clear')"
    ...
    <div v-if="workspaceVariant && !streaming" class="workspace-actions">
      <button type="button" class="primary" @click="sendPreset('polish')">
        ✦ {{ t('aiPanel.workspacePolish') }}
      </button>
      <button type="button" @click="sendPreset('expand')">
        ↗ {{ t('aiPanel.workspaceExpand') }}
      </button>
      <button type="button" @click="sendPreset('rewrite')">
        ↻ {{ t('aiPanel.workspaceRewrite') }}
      </button>
      <button type="button" @click="sendCompliance">
        ✓ {{ t('aiPanel.workspaceCompliance') }}
      </button>
    </div>
    ...
    <div class="ac-messages" ref="messagesRef" @click="handleCodeBlockClick">
      <Transition name="v-fade">
        <div v-if="messages.length === 0 && !streaming" class="ac-empty">
          ...
          <p
            class="ac-empty-sub anim-fade-in-up"
            v-html="t('aiPanel.greetingHint', { atChar: '@' })"
          ></p>
        </div>
      </Transition>
        ...
        <div class="ac-body">
          ...
          <template v-else>
            ...
            <div v-if="!msg.isStreaming && msg.content" class="ac-actions">
              ...
              <button
                class="ac-action-btn u-interactive"
      ...
      <div v-if="streaming" class="ac-msg assistant">
        ...
        <div class="ac-body">
          ...
          <div
            v-if="streamContent"
            class="ac-ai-bubble"
            v-html="renderMd(streamContent, 'streaming')"
          ></div>
    ...
    <div class="ac-attachments" v-if="files.length">
      <TransitionGroup name="v-spring">
        <div class="ac-file u-interactive" v-for="f in files" :key="f.name">
          ...
          <button
            class="ac-file-x"
            :title="t('aiPanel.removeAttachment')"
    ...
    <Transition name="v-slide-up">
      <div class="ac-presets anim-stagger" v-if="!streaming">
        <button
          class="ac-preset u-interactive"
          style="--stagger-i: 0"
          @click="sendPreset('polish')"
        >
          {{ t('aiPanel.presetPolish') }}
        </button>
        <button
          class="ac-preset u-interactive"
          style="--stagger-i: 1"
        ...
        >
          {{ t('aiPanel.presetExpand') }}
        </button>
        <button
          class="ac-preset u-interactive"
          style="--stagger-i: 2"
          @click="sendPreset('review')"
        >
          {{ t('aiPanel.presetReview') }}
        </button>
        <button class="ac-preset u-interactive" style="--stagger-i: 3" @click="sendPreset('en')">
          {{ t('aiPanel.presetEn') }}
        </button>
        <button class="ac-preset u-interactive" style="--stagger-i: 4" @click="sendPreset('zh')">
          {{ t('aiPanel.presetZh') }}
        </button>
      </div>
    </Transition>
    ...
    <div class="ac-input-area">
        ...
        <Transition name="v-scale-in">
          <div v-if="slashMenu" class="ac-menu ac-slash-menu">
            <div
              ...
              :key="cmd.cmd"
              class="ac-menu-item anim-fade-in-up anim-stagger"
              :style="{ '--stagger-i': i }"
      ...
      <button
        ...
        :title="
          panelSpeech.status.value === 'listening'
            ? t('aiPanel.voiceStop')
...
const commands = [
  {
    cmd: '/explain',
    prompt: 'Please explain the following text in detail, breaking down the key concepts:',
    desc: t('aiPanel.cmd.explain'),
  },
  {
    cmd: '/fix',
    prompt: 'Please identify and fix any issues in the following text:',
    desc: t('aiPanel.cmd.fix'),
  },
  {
    cmd: '/translate',
    prompt: 'Please translate the following text:',
    desc: t('aiPanel.cmd.translate'),
  },
  {
    cmd: '/polish',
    prompt: 'Please polish the following academic text, improving grammar, vocabulary and style:',
    desc: t('aiPanel.cmd.polish'),
  },
  {
    cmd: '/expand',
    prompt: 'Please expand the following text with more details and academic depth:',
    desc: t('aiPanel.cmd.expand'),
  },
  {
    cmd: '/summarize',
    prompt: 'Please provide a concise summary of the following text:',
    desc: t('aiPanel.cmd.summarize'),
  },
  {
    cmd: '/outline',
    prompt: 'Please generate a detailed outline for a paper on the following topic:',
    desc: t('aiPanel.cmd.outline'),
  },
  {
    cmd: '/review',
    prompt: 'Please review the following academic text, identify issues and suggest improvements:',
    desc: t('aiPanel.cmd.review'),
  },
  {
    cmd: '/rewrite',
    prompt: 'Please rewrite the following text to improve clarity and flow:',
    desc: t('aiPanel.cmd.rewrite'),
  },
  {
    cmd: '/cite',
    prompt:
      'Please generate proper academic citations (IEEE/APA/GB-T) for the following references:',
    desc: t('aiPanel.cmd.cite'),
  },
]
...
function renderMd(text: string, msgId: string): string {
  ...
  h = h.replace(/```(\w*)\n([\s\S]*?)```/g, (_match, lang: string, code: string) => {
    ...
    return (
      `<div class="ac-code-block" data-id="${id}">` +
...
async function sendCompliance() {
  input.value = t('aiPanel.compliancePrompt')
  await send()
}
...
async function sendPreset(action: string) {
  ...
  const instructions: Record<string, string> = {
    polish: t('aiPanel.prompts.polish'),
    expand: t('aiPanel.prompts.expand'),
    rewrite: t('aiPanel.prompts.rewrite'),
    review: t('aiPanel.prompts.review'),
    en: t('aiPanel.prompts.en'),
    zh: t('aiPanel.prompts.zh'),
  }
  ...
  try {
    ...
    await readSseStream(
      ...
      (eventType, event) => {
        if (eventType === 'error') {
          streamContent.value =
            (event.message as string) || (event.content as string) || t('aiPanel.error')
    ...
    streamContent.value = t('aiPanel.errorPrefix', {
  ...
  try {
    ...
    await readSseStream(reader, (evtType, d) => {
      ...
      if (evtType === 'token' && d.content) {
        ...
        streamContent.value = (d.content as string)
          .replace(/<think\b[^>]*>[\s\S]*?<\/think\s*>/g, '')
          .trim()
        tokenBuffer = ''
      } else if (evtType === 'error') {
        streamContent.value = (d.content as string) || t('aiPanel.error')
      } else if ((evtType === 'thought' || evtType === 'thinking') && d.content) {
        thinkingText.value = d.content as string
      } else if (evtType === 'tool_call') {
        hasToolActivity = true
        tokenBuffer = ''
        thinkingText.value = t('aiPanel.calling', {
          tool: (meta?.tool_name as string) || (meta?.tool as string) || '...',
        })
        ...
        if ((tool === 'str_replace' || tool === 'write_file') && filePath) {
          const open = editorTabs.value.some(
            (t) => t.path && normPath(t.path) === normPath(filePath),
          )
          if (open) {
            ...
            setActiveEdit({
              ...
              operation: (tool === 'write_file' ? 'write_file' : 'str_replace') as
...
function handleCodeBlockClick(e: MouseEvent) {
  ...
  if (btn.classList.contains('ac-code-copy')) {
    ...
    setTimeout(() => {
      btn.textContent = t('aiPanel.copy')
    }, 1500)
...
.ac-icon-btn:not(:disabled):hover {
...
.ac-empty-icon {
  ...
  color: var(--c-accent);
  opacity: 0.55;
  animation: ac-float 3.6s var(--ease-smooth) infinite;
}
@keyframes ac-float {
  0%,
  100% {
...
.ac-msg {
  ...
  gap: var(--space-2);
  max-width: 100%;
  animation: ac-msg-in var(--motion-slow) var(--ease-out) both;
}
@keyframes ac-msg-in {
  from {
    opacity: 0;
...
.ac-msg.user .ac-avatar {
  background: var(--c-accent);
  color: #fff;
}
.ac-msg.assistant .ac-avatar {
  background: var(--c-surface-4);
  color: var(--c-text-0);
...
:deep(.ac-code-btn):hover {
  background: var(--c-accent-soft);
}
:deep(.ac-code-btn):active {
  transform: scale(0.94);
}
:deep(.ac-code-block pre) {
  background: var(--c-surface-2);
  color: var(--c-text-0);
  padding: var(--space-3);
  margin: 0;
...
.ac-action-btn:not(:disabled):hover {
  background: var(--c-surface-3);
  color: var(--c-text-0);
  border-color: var(--c-accent);
}
.ac-action-btn:focus-visible {
  outline: none;
  box-shadow: var(--ring-focus);
  border-color: var(--c-accent);
}
.ac-action-btn.copied {
  color: var(--c-success);
...
.ac-preset:not(:disabled):hover {
  border-color: var(--c-accent);
  color: var(--c-text-0);
  background: var(--c-accent-soft);
}
.ac-preset:focus-visible {
  outline: none;
  box-shadow: var(--ring-focus);
  border-color: var(--c-accent);
}
...
.ac-attach-btn:not(:disabled):hover {
  background: var(--c-surface-3);
  color: var(--c-text-0);
  border-color: var(--c-accent);
}
.ac-attach-btn:focus-visible {
  outline: none;
  box-shadow: var(--ring-focus);
  border-color: var(--c-accent);
}
.ac-attach-btn:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
.ac-attach-btn.voice-active {
  background: var(--c-accent);
  color: #fff;
  border-color: var(--c-accent);
...
.ac-send-btn:not(:disabled):hover {
  background: var(--c-accent-hover);
}
.ac-send-btn:focus-visible {
  outline: none;
  box-shadow: var(--ring-focus);
}
...
.ac-send-btn.stopping {
  background: var(--c-danger);
  animation: stop-ring 1.8s ease-in-out infinite;
}
@keyframes stop-ring {
  0%,
  100% {
    box-shadow: 0 0 0 0 var(--c-danger-border);
  }
  50% {
...
.workspace-variant .ac-header {
  height: 54px;
  padding: 0 15px;
  border-color: var(--c-border);
  background: var(--c-panel);
}
.workspace-variant .ac-title {
  color: var(--c-text-0);
  font-size: 14px;
  font-weight: 650;
}
.workspace-variant .ac-context {
  margin: 10px 12px 0;
  padding: 7px 9px;
...
.workspace-variant .ac-empty {
  display: none;
}
.workspace-variant .ac-messages {
  padding: 0 12px 12px;
}
.workspace-variant .ac-presets {
  display: none;
}
.workspace-variant .ac-input-area {
  margin: 0 10px 10px;
  border: 1px solid var(--c-border);
  border-radius: 9px;
  background: var(--c-panel);
