<template>
  <div class="arg-inspector">
    ...
    <template v-if="!selectedNode && !selectedEdge">
      ...
      <div v-if="state.graph?.issues?.length" class="inspector-field">
        <label class="inspector-label">{{
          t('argument.issuesLabel', { count: state.graph.issues.length })
        }}</label>
        <div
      ...
      <div class="inspector-actions">
        <button
          ...
          @click="doCritique"
        >
          {{ state.critiquing ? t('argument.critiquing') : t('argument.critique') }}
        </button>
        <button class="inspector-btn" :disabled="!state.graph || exporting" @click="doExport">
          {{ exporting ? t('argument.exporting') : t('argument.exportDraft') }}
        </button>
        <button class="inspector-btn" @click="$emit('auto-layout')">
    ...
    <template v-else-if="selectedNode">
      <div class="inspector-section">
        ...
        <div class="inspector-field">
          ...
          <textarea v-model="editText" class="inspector-textarea" rows="4" @blur="commitText" />
        </div>

        <div v-if="selectedNode.issue_ids?.length" class="inspector-field">
          <label class="inspector-label">{{
            t('argument.issuesLabel', { count: selectedNode.issue_ids.length })
          }}</label>
          <div
        ...
        <div v-if="selectedNode" class="inspector-field">
          <label class="inspector-label">{{
            t('argument.sourceQuote', { count: nodeSpans.length })
          }}</label>
          <div v-if="!nodeSpans.length" class="placeholder-text">
            {{ t('argument.selectSentence') }}
          </div>
          <div v-for="span in nodeSpans" :key="span.id" class="inspector-span">
            <span class="span-quote"
              >「{{ span.quote.slice(0, 36) }}{{ span.quote.length > 36 ? '…' : '' }}」</span
            >
            <div class="span-actions">
              <button
                class="span-action-btn"
                :title="span.source_label || span.side"
                @click="jumpToSpan(span.id)"
              >
                {{ t('argument.jumpToSource') }}
              </button>
              <button class="span-action-btn span-action-btn--danger" @click="removeSpan(span.id)">
                {{ t('argument.unbind') }}
              </button>
            </div>
          </div>
        ...
        <div v-if="suggestResult" class="inspector-field">
          <label class="inspector-label">{{
            t('argument.aiSuggestions', { count: suggestResult.candidates.length })
          }}</label>
          <div v-for="c in suggestResult.candidates" :key="c.local_id" class="suggest-candidate">
            <div class="candidate-type" :class="`type-${c.node_type}`">
              {{ typeLabel(c.node_type) }}
            </div>
            <div class="candidate-text">{{ c.text }}</div>
            <button class="candidate-adopt-btn" @click="adoptCandidate(c)">
              {{ t('argument.adopt') }}
            </button>
          </div>
          <button
      ...
      <div class="inspector-actions">
        <button class="inspector-btn" :disabled="!selectedNode || suggesting" @click="doSuggest">
          {{ suggesting ? t('argument.suggestingNext') : t('argument.suggestNext') }}
        </button>
        <button class="inspector-btn inspector-btn--danger" @click="deleteSelectedNode">
          {{ t('argument.deleteNode') }}
        </button>
      </div>
    </template>
    ...
    <template v-else-if="selectedEdge">
      ...
      <div class="inspector-actions">
        <button class="inspector-btn inspector-btn--danger" @click="deleteSelectedEdge">
          {{ t('argument.deleteEdge') }}
        </button>
      </div>
    </template>
  </div>
...
async function doExport() {
  ...
  try {
    ...
    if (!res.body) {
      exportMsg.value = t('argument.requestFailed')
      return
    }
    ...
    await readSseStream(res.body.getReader(), (eventType, data) => {
      if (eventType === 'complete') {
        ...
        exportMsg.value = t('argument.exportError', {
...
function relLabel(r: RelationType) {
  return REL_LABELS[r] ?? r
}
...
.inspector-node-type-badge.type-claim {
  color: var(--c-accent);
}
.inspector-node-type-badge.type-grounds {
  color: #10b981;
}
.inspector-node-type-badge.type-warrant {
  color: #3b82f6;
}
.inspector-node-type-badge.type-backing {
  color: #93c5fd;
}
.inspector-node-type-badge.type-qualifier {
  color: #f59e0b;
}
.inspector-node-type-badge.type-rebuttal {
  color: var(--c-danger);
}
...
.inspector-issue.sev-error {
  color: var(--c-danger);
}
.inspector-issue.sev-warning {
  color: var(--c-warn-fg);
}
.inspector-issue.sev-info {
  color: var(--c-text-2);
}
.issue-sev {
  font-size: 10px;
...
.span-action-btn--danger {
  color: var(--c-danger);
}
.span-action-btn--danger:hover {
...
.inspector-edge-rel.rel-supports {
  color: #10b981;
}
.inspector-edge-rel.rel-warrants {
  color: #3b82f6;
}
.inspector-edge-rel.rel-backs {
  color: #93c5fd;
}
.inspector-edge-rel.rel-qualifies {
  color: #f59e0b;
}
.inspector-edge-rel.rel-rebuts {
  color: var(--c-danger);
}
.inspector-edge-rel.rel-counters {
  color: #f97316;
}
...
.candidate-type.type-claim {
  color: var(--c-accent);
}
.candidate-type.type-grounds {
  color: #10b981;
}
.candidate-type.type-warrant {
  color: #3b82f6;
}
.candidate-type.type-backing {
  color: #93c5fd;
}
.candidate-type.type-qualifier {
  color: #f59e0b;
}
.candidate-type.type-rebuttal {
  color: var(--c-danger);
}
