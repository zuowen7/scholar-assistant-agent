    ...
    <div class="sidebar-footer">
      ...
      <div class="user-row">
        ...
        <div class="user-actions">
          <button type="button" class="settings-button" :title="t('topbar.settings')" :aria-label="t('topbar.settings')" @click="$emit('settings')"><Settings :size="18" aria-hidden="true" /></button>
        </div>
      </div>
    </div>
  </aside>
...
.nav-item { position: relative; width: 100%; height: 44px; display: flex; align-items: center; gap: 12px; padding: 0 12px; border: 0; border-radius: 8px; background: transparent; color: var(--c-text-1); font: 500 14px/1 var(--font-sans), var(--font-zh); cursor: pointer; transition: color var(--motion-fast), background var(--motion-fast); }
...
.nav-item.active::before { content: ''; position: absolute; left: 0; width: 3px; height: 18px; border-radius: 0 3px 3px 0; background: var(--c-accent); }
.nav-item:focus-visible, .settings-button:focus-visible { outline: none; box-shadow: var(--ring-focus); }
...
.settings-button { margin-left: auto; width: 32px; height: 32px; display: grid; place-items: center; border: 0; border-radius: 7px; background: transparent; color: var(--c-text-2); cursor: pointer; transition: color var(--motion-fast), background var(--motion-fast); }
