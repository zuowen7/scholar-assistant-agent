> ⚠️ **STALE / 已过时（2026-07-20 核实）**：本文档列出的"致命问题"（无真流式、无
> PermissionEnforcer、无 per-turn 自动保存、无系统提示词构建、dispatch_tool 路径穿越、
> 无上下文压缩、无错误恢复）**均已在代码中修复**，与 AGENTS.md 的 A 级评级和实际实现
> 冲突。保留本文仅作历史记录，**请勿据此判断现状**。权威说明见仓库根 `AGENTS.md` 与
> `CLAUDE.md`。待清理：可在确认无引用后删除本文件。
