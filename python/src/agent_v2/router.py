...
def _build_system_prompt(workspace_root: str, tools: list) -> str:
    ...
    return (
        ...
        f"CRITICAL: When expanding documents, work ONE SECTION at a time.\n"
        f"Each str_replace call must replace a single section (e.g., Abstract, Introduction).\n"
        f"old_string must be the EXACT current text of one section (keep it under 300 chars).\n"
        f"new_string must expand ONLY that section (keep it under 500 words).\n"
        f"NEVER expand the entire document in one call — it will timeout.\n"
        f"After expanding one section, move to the next one.\n"
