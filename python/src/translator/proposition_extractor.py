...
_LOGIC_PATTERNS: list[tuple[re.Pattern, str, str]] = [
    ...
    (
        re.compile(r"(?:归因于|源于|取决于)"),
        "attributed to / stem from / depend on",
        "cause-effect",
    ),
    # 对比/转折
    ...
    # 含义/推论
    (
        re.compile(r"(?:这意味着|这说明|这暗示|这表明|由此可见)"),
        "this suggests / this indicates / implying that",
        "implication",
    ),
    (re.compile(r"(?:换言之|换句话说|也就是说)"), "in other words / that is", "clarification"),
    # 局限/让步
    (
        re.compile(r"(?:需要注意的是|应当指出|必须承认)"),
        "it should be noted that / it must be acknowledged that",
        "limitation",
    ),
