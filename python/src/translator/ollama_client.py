...
class OllamaClient:
    ...
    def chat(
        ...
        """
        直接调用 Ollama /api/chat，不经过翻译专用 prompt 包装。
        ...
        try:
            ...
            raise ValueError(f"Ollama 请求失败: HTTP {e.response.status_code}") from e
        except httpx.TimeoutException as e:
            raise ConnectionError(f"Ollama 请求超时 ({self.timeout}s)") from e

        data = resp.json()
        raw = (data.get("message") or {}).get("content") or ""
        return _strip_preamble(_strip_context_leak(raw.strip()))
