...
PROVIDER_PRESETS: dict[str, dict] = {
    ...
    "zhipu": {
        ...
        "models": ["glm-4-plus", "glm-4-flash", "glm-4-long", "glm-5.1", "glm-5"],
        "api_format": "openai",
    },
    "qwen": {
        "name": "通义千问",
...
class CloudClient:
    ...
    def chat(
        ...
        """
        直接调用 LLM Chat API，不经过翻译专用 prompt 包装。

        Args:
            messages: [{"role": "user"|"assistant", "content": "..."}, ...]
            system_prompt: 系统提示词
            max_tokens: 最大生成 token 数（默认用 self.max_tokens）
        ...
        """
        if self.api_format == "anthropic":
            return self._chat_anthropic(messages, system_prompt, max_tokens or self.max_tokens)
        return self._chat_openai(messages, system_prompt, max_tokens or self.max_tokens)
    ...
    def _chat_anthropic(
        ...
        for msg in messages:
            ...
            if role == "system":
                ...
                continue
            if role == "assistant":
                anthropic_messages.append({"role": "user", "content": [{"type": "user", "content": "placeholder"}]})
    ...
    def _openai_health_check(self) -> bool:
        ...
        # 1) 尝试 /models 端点（不消耗 token）— 只用 base_url/models，不拼 /v1
        try:
            resp = client.get(f"{self.base_url}/models", headers=headers, timeout=10.0)
        ...
        # 2) fallback: 发最小 chat 请求（兼容不支持 /models 的网关）
        url = f"{self.base_url}/chat/completions"
