...
def _crawl_arxiv(query: str, max_results: int = 5) -> str:
    ...
    try:
        ...
        # 从应用配置或环境变量读取代理
        from api_factory import get_proxy
        proxy_url = get_proxy()
...
def _polish_text(text: str, style: str = "academic") -> str:
    """润色文本，使其更加学术化、地道、专业。不改变原意，只优化表达。

    Args:
        text: 待润色的文本内容。
        style: 润色风格。academic=学术论文风格, concise=精简, fluent=流畅自然。
    """
    raise NotImplementedError("润色功能需要 LLM 客户端")


def _generate_outline(topic: str, sections: str = "") -> str:
    """根据研究主题生成学术论文大纲。自动生成符合顶会规范的章节结构。

    Args:
        topic: 研究主题或核心 Idea。
        sections: 指定需要的章节（逗号分隔），如 "Introduction,Method,Experiment"。留空则自动生成完整大纲。
    """
    raise NotImplementedError("大纲生成需要 LLM 客户端")


def _summarize_text(text: str, max_sentences: int = 5) -> str:
    """对长文本进行摘要。提取核心观点，生成简洁的摘要。

    Args:
        text: 待摘要的文本内容。
        max_sentences: 摘要的最大句数，默认 5。
    """
    raise NotImplementedError("摘要功能需要 LLM 客户端")


def _save_file(file_path: str, content: str) -> str:
    """将文本内容保存到指定文件。支持创建新文件或覆盖已有文件。
    ...
    try:
        ...
        path = Path(file_path)
        # 路径安全检查
        _DENIED = ("/etc", "/proc", "/sys", "/dev", "/root",
                    "C:\\Windows", "C:\\Program Files")
        resolved = str(path.resolve())
        for prefix in _DENIED:
            if resolved.startswith(prefix):
                return f"错误: 禁止写入系统目录 {prefix}"
        if path.suffix.lower() in {".env", ".key", ".pem", ".exe", ".bat", ".cmd", ".ps1"}:
            return f"错误: 禁止写入 {path.suffix} 文件"
...
def create_default_registry(
    ...
    Args:
        ...
        paper_rag_store: 论文模板专用 RAGStore 实例（可选）。
        llm_call_fn: LLM 调用函数 (prompt) -> response，用于润色/大纲/摘要等工具。
    ...
    # --- 论文模板检索工具 ---
    if paper_rag_store is not None:

        def search_paper_templates(query: str, top_k: int = 5) -> str:
            """检索论文模板和写作范例。从内置的论文模板库中搜索相关的格式要求、写作范例和章节模板。

            Args:
                query: 查询内容，如"IEEE introduction 怎么写"或"NeurIPS 格式要求"。
                top_k: 返回的最大结果数量，默认 5。
            """
            try:
                results = paper_rag_store.retrieve_context(query, top_k=top_k)
                if not results:
                    return "未找到相关模板内容。"
                parts: list[str] = []
                for i, r in enumerate(results):
                    ...
                    label_parts = [p for p in [template, section, category] if p]
                    label = f" ({', '.join(label_parts)})" if label_parts else ""
                    parts.append(f"[模板片段 {i + 1}]{label}\n{r['text']}")
                return "\n\n---\n\n".join(parts)
            except Exception as e:
                return f"模板检索失败: {e}"

        search_paper_templates._agent_tool_def = ToolDefinition(
            name="search_paper_templates",
            description="检索论文模板和写作范例。从内置的论文模板库中搜索相关的格式要求、写作范例和章节模板。",
            parameters=_extract_schema_from_function(search_paper_templates),
            fn=search_paper_templates,
        )
        registry.register(search_paper_templates)

    # --- 文件读写工具 ---
    registry.register(ToolDefinition(
        name="save_file",
        description="将文本内容保存到指定文件。支持创建新文件或覆盖已有文件。",
    ...
    if llm_call_fn is not None:
        ...
        def polish_text(text: str, style: str = "academic") -> str:
            ...
            """
            style_map = {
                "academic": "请以顶级期刊/会议论文的标准润色以下文本，使用更精确的学术术语，改善句式结构，但不改变原意。只输出润色后的文本。",
                "concise": "请精简以下文本，去除冗余表达，保留核心信息，使表达更加简洁有力。只输出精简后的文本。",
                "fluent": "请改善以下文本的流畅度和可读性，使表达更加自然地道，但不改变原意。只输出改写后的文本。",
            }
            prompt = style_map.get(style, style_map["academic"]) + f"\n\n{text}"
            try:
        ...
        registry.register(ToolDefinition(
            name="polish_text",
            description="润色文本，使其更加学术化、地道、专业。不改变原意，只优化表达。支持 academic/concise/fluent 三种风格。",
            parameters=_extract_schema_from_function(polish_text),
            fn=polish_text,
        ))

        def generate_outline(topic: str, sections: str = "") -> str:
            """根据研究主题生成学术论文大纲。自动生成符合顶会规范的章节结构。
            ...
            """
            prompt = (
                "你是一位资深学术论文写作指导专家。请为以下研究主题生成一份详细的论文大纲。\n"
                "大纲应包含各章节的标题和每节需要涵盖的关键内容（2-3 句描述）。\n"
                "使用 Markdown 格式输出。\n"
            )
            if sections:
                prompt += f"用户指定章节: {sections}\n"
            else:
                prompt += "请生成完整大纲（Abstract, Introduction, Related Work, Method, Experiments, Results, Discussion, Conclusion）。\n"
            prompt += f"\n研究主题: {topic}"
            try:
                return llm_call_fn(prompt)
            except Exception as e:
        ...
        registry.register(ToolDefinition(
            ...
            description="根据研究主题生成学术论文大纲。自动生成符合顶会规范的章节结构。",
        ...
        def summarize_text(text: str, max_sentences: int = 5) -> str:
            ...
            """
            prompt = (
                f"请对以下文本进行摘要，提取核心观点，控制在 {max_sentences} 句以内。"
                "只输出摘要内容，不要添加额外说明。\n\n"
                f"{text[:6000]}"
            )
            try:
        ...
        registry.register(ToolDefinition(
            name="summarize_text",
            description="对长文本进行摘要。提取核心观点，生成简洁的摘要。",
            parameters=_extract_schema_from_function(summarize_text),
            fn=summarize_text,
        ))

    logger.info("工具注册完成: %s", [t.name for t in registry.list_tools()])
    return registry
