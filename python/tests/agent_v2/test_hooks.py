...
class TestHookDefinition:
    def test_create_hook(self):
        h = HookDefinition(
            name="test", hook_point=HookPoint.PRE_TOOL_USE, command="echo ok", priority=30
        )
        assert h.name == "test"
    ...
    def test_to_dict(self):
        h = HookDefinition(
            name="h1", hook_point=HookPoint.POST_TOOL_USE, command="echo x", priority=50
        )
        d = h.to_dict()
        assert d["name"] == "h1"
...
class TestHookEdge:
    ...
    async def test_very_long_input(self):
        ...
        event = HookEvent(hook=HookPoint.POST_TOOL_USE, tool_name="test", tool_input="x" * 100_000)
        result = await runner.run(HookPoint.POST_TOOL_USE, event)
        assert result.decision == HookDecision.ALLOW
