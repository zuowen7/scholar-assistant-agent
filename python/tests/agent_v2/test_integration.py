    ...
    async def test_ie001_read_and_summarize(self, workspace: Path):
        ...
        provider = MockProvider(
            scenarios=[
                Scenario(
                    "r0",
                    trigger_patterns=["read and summarize"],
                    turn_index=0,
                    response_factory=lambda m, t: _tool_response(
                        "read_file", {"file_path": "paper.md"}
                    ),
                ),
                Scenario(
                    "r1",
    ...
    async def test_ie004_modify_paper(self, workspace: Path):
        ...
        provider = MockProvider(
            scenarios=[
                Scenario(
                    "fix",
                    trigger_patterns=["fix", "modify", "replace"],
                    response_factory=lambda m, t: _tool_response(
                        "str_replace",
                        {
                            "file_path": "paper.md",
...
class TestPermissionFlows:
        ...
        provider = MockProvider(
            scenarios=[
                Scenario(
                    ...
                    response_factory=lambda m, t: _tool_response(
                        "write_file", {"file_path": "new.txt", "content": "hello"}
                    ),
                ),
            ]
        )
    ...
    async def test_ie011_path_traversal_denied(self, workspace: Path):
        ...
        tool_results = [
            e for e in events if e.type in (AgentEventType.TOOL_RESULT, AgentEventType.TOOL_ERROR)
        ]
...
class TestMcpIntegration:
    ...
    async def test_ie002_translate_via_mcp(self, workspace: Path):
        ...
        mcp = McpManager(
            configs=[
                McpServerConfig(
                    name="scholar", command=sys.executable, args=[str(script)], timeout_seconds=5.0
                ),
            ]
        )
