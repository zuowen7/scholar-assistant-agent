...
class TestBashValidationEdge:
    ...
    def test_validate_command_blocks_git_push_in_readonly(self):
        ...
        result = self.validate_command(
            "git push origin main", PermissionMode.READ_ONLY, Path("/ws")
        )
...
class TestHooksEdge:
    ...
    async def test_hook_with_malformed_json_stdout(self):
        from src.agent_v2.hooks import HookDefinition, HookEvent, HookPoint, HookRunner
    ...
    async def test_hook_with_very_large_input(self):
        ...
        event = HookEvent(hook=HookPoint.PRE_TOOL_USE, tool_name="test", tool_input="x" * 1_000_000)
        result = await runner.run(HookPoint.PRE_TOOL_USE, event)
        assert result.decision == HookDecision.ALLOW

    @pytest.mark.asyncio
    async def test_abort_signal_after_completion(self):
        from src.agent_v2.hooks import HookAbortSignal, HookEvent, HookPoint, HookRunner
...
class TestTridentEdge:
    ...
    def test_collapse_with_all_tool_messages(self):
        ...
        messages = [
            Message(
                ...
                blocks=[ToolUseBlock(id="t1", name="read_file", input="{}")],
            ),
            Message(
                role=MessageRole.TOOL,
                blocks=[ToolResultBlock(tool_use_id="t1", tool_name="read_file", output="content")],
            ),
        ] * 5
    ...
    def test_supersede_with_mixed_case_paths(self):
        ...
        messages = [
            Message(
                role=MessageRole.ASSISTANT,
                blocks=[ToolUseBlock(id="t1", name="read_file", input='{"file_path": "File.txt"}')],
            ),
            Message(
                role=MessageRole.TOOL,
                blocks=[ToolResultBlock(tool_use_id="t1", tool_name="read_file", output="content")],
            ...
            Message(
                ...
                blocks=[
                    ToolUseBlock(
                        id="t2",
                        name="write_file",
                        input='{"file_path": "file.txt", "content": "x"}',
                    )
                ],
            ),
            Message(
...
class TestPolicyEngineEdge:
    ...
    def test_threshold_with_non_numeric(self):
        ...
        engine = PolicyEngine()
        engine.add_rule(
            PolicyRule("r1", PolicyCondition(key="x", threshold=10), PolicyAction.ESCALATE)
        )
    ...
    def test_many_rules(self):
        ...
        for i in range(100):
            engine.add_rule(
                PolicyRule(
                    f"r{i}", PolicyCondition(key="x", value=True), PolicyAction.LOG, priority=i
                )
            )
...
class TestPolicyEngineEdgeMore:
    ...
    def test_threshold_exact_match(self):
        ...
        engine = PolicyEngine()
        engine.add_rule(
            PolicyRule("r1", PolicyCondition(key="score", threshold=10), PolicyAction.ESCALATE)
        )
    ...
    def test_duplicate_priorities(self):
        ...
        engine = PolicyEngine()
        engine.add_rule(
            PolicyRule("a", PolicyCondition(key="x", value=True), PolicyAction.LOG, priority=10)
        )
        engine.add_rule(
            PolicyRule("b", PolicyCondition(key="x", value=True), PolicyAction.RETRY, priority=10)
        )
        actions = engine.evaluate({"x": True})
    ...
    def test_multiple_conditions_conflicting(self):
        ...
        engine = PolicyEngine()
        engine.add_rule(
            PolicyRule("log", PolicyCondition(key="x", value=True), PolicyAction.LOG, priority=1)
        )
        engine.add_rule(
            PolicyRule(
                "abort", PolicyCondition(key="x", value=True), PolicyAction.ABORT, priority=2
            )
        )
        actions = engine.evaluate({"x": True})
...
class TestLspEdgeMore:
    ...
    def test_state_transition_to_error(self):
        from src.agent_v2.runtime.lsp_client import LspClientState, LspRegistry
...
class TestTridentEdgeMore:
    ...
    def test_stage3_similar_texts_cluster(self):
        ...
        msgs = [
            ...
            Message(
                role=MessageRole.ASSISTANT, blocks=[TextBlock(text="apple banana date elderberry")]
            ),
        ]
    ...
    def test_stage1_supersede_multiple_writes_same_file(self):
        ...
        msgs = [
            Message(
                ...
                blocks=[
                    ToolUseBlock(
                        id="t1", name="write_file", input=json.dumps({"file_path": "f.txt"})
                    )
                ],
            ),
            Message(
                role=MessageRole.TOOL,
                blocks=[ToolResultBlock(tool_use_id="t1", tool_name="write_file", output="ok")],
            ),
            Message(
                role=MessageRole.ASSISTANT,
                blocks=[
                    ToolUseBlock(
                        id="t2", name="write_file", input=json.dumps({"file_path": "f.txt"})
                    )
                ],
            ),
            Message(
    ...
    def test_stage1_read_not_superseded(self):
        ...
        msgs = [
            Message(
                ...
                blocks=[
                    ToolUseBlock(
                        id="t1", name="read_file", input=json.dumps({"file_path": "f.txt"})
                    )
                ],
            ),
            Message(
                role=MessageRole.TOOL,
                blocks=[ToolResultBlock(tool_use_id="t1", tool_name="read_file", output="v1")],
            ),
            Message(
                role=MessageRole.ASSISTANT,
                blocks=[
                    ToolUseBlock(
                        id="t2", name="read_file", input=json.dumps({"file_path": "f.txt"})
                    )
                ],
            ),
            Message(
                role=MessageRole.TOOL,
                blocks=[ToolResultBlock(tool_use_id="t2", tool_name="read_file", output="v2")],
            ),
        ]
        kept, count = stage1_supersede(msgs)
...
class TestGitContextEdgeMore:
    ...
    def test_render_with_commits_no_branch(self):
        ...
        ctx = GitContext(
            branch=None,
            recent_commits=[GitCommitEntry(hash="abc1234", subject="fix: bug")],
            staged_files=[],
        )
        rendered = ctx.render()
...
class TestHookRunResultEdgeMore:
    def test_is_allowed_and_is_denied_mutually_exclusive(self):
        from src.agent_v2.hooks import HookDecision, HookRunResult
