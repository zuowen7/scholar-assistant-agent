...
- **2026-05-18 Bug fix pass**：修 5 个 bug（`_save_skill` 无 frontmatter 回退写错文件 + `_skill_migrate` 空白 name fallback + `_parse_llm_points` 缺 category 静默默认 + 模板占位符未替换 + `_reviewer_perspectives` 的 `{venue}/{text}` 未替换）；新增 40 个 adversarial 测试；回归 1566 passed / 8 skipped
- **2026-05-18 Integration wiring**：`run_review_parallel` 接入 `POST /api/companion/review`（`mode: "parallel"` + `features.parallel_review: false` 灰度开关）；`run_review_parallel` 补 `focus`/`checks` 参数（focus 委托给串行 `run_review`）；前端 TypeScript `ReviewPoint` 加 `perspective` 字段；回归 1559 passed / 8 skipped + vitest 326 passed
