...
## 14. 2026-07-30 真实长稿审查回归：从“拦截”改为正常流程收敛

### 14.1 修正结论

第 13 节的广度测试不足以覆盖用户这次实际组合：31,516 字符的完整编辑器快照、陈旧 Claim Ledger、缺失论证图、Reviewer 查询参数错误，以及本地小模型连续改写。会话 `sess_10d78c45f1574f4e8fe175c9a72124b0` 因此仍在 4 次校正后进入 `FAILED / response_grounding_failed`。这不是“脏编辑器”造成的：该会话明确传入 `dirty=false`，完整正文也已进入当前 turn。

本轮稳定性目标不是把失败包装成降级成功，而是建立以下正常路径不变量：

1. 审稿前由 runtime 自动取得当前文档对应的 Graph、Ledger 和 Reviewer 状态，不依赖模型猜参数。
2. 缺失能力是可陈述的证据状态，不是让整个审稿任务终止的工具错误。
3. 陈旧 Ledger 只能作为待核查线索，不能给当前稿件事实背书。
4. 每次校正必须累积此前全部违规项，不能只修当前一轮后再编造一批新数字。
5. Nature 审稿只有包含严重级别和当前稿件原文锚点时才能交付 `COMPLETE`。

### 14.2 失败事实与根因

真实会话先后调用 Graph、Ledger 和 Reviewer；Graph 不存在，Ledger 显示 `stale=true`，Reviewer 却使用猜测的 `session_id=default` 并得到 404。随后模型重复分页读取陈旧 Ledger，而没有形成可定位到当前正文的审稿意见。

使用同一份本地论文和本机 `qwen3:8b` 回放旧路径时，4 次草稿依次引入：

- `56.8`、`123`、`66.4`、`76.8`、`78.4`、`82`、`79.5`、`90`、`81.2`、`105`；
- 4 个未提供的 arXiv 标识符；
- `300`、`94.1%`；
- `120`。

旧校正提示只禁止当轮检测到的事实，模型因而能在下一轮换一批新事实，最终硬失败。另一方面，旧完成条件只检查“是否有无来源事实”，没有检查答复是否真的包含原文锚点和严重程度；因此后续一次本地回放虽达到 `COMPLETE` 且工具错误为 0，答复仍只是 Graph/Ledger/Reviewer 状态摘要，不足以成为投稿前审稿结果。

### 14.3 正常流程修复

| 层面 | 修复后的不变量 |
|---|---|
| 编辑器上下文 | 当前 turn 标记 `complete_editor_snapshot`、文件路径、内容哈希和 dirty 状态；持久化 session 仍不复制整篇私密正文 |
| 学术状态预取 | `nature_reviewer` 在首次模型调用前按当前 `doc_id` 自动读取 Graph、两页 Ledger promises 和 Reviewer points |
| Reviewer 定位 | 传 `doc_id` 且只匹配到一个 Reviewer session 时自动展开该 session；禁止猜 `default` |
| 能力缺失 | Graph 或 Reviewer 不存在时返回 `available=false` 的成功状态包络，模型可如实说明限制并继续审稿 |
| 陈旧数据 | `stale=true` 的 Ledger 明细不进入当前稿件事实来源集合，只保留状态和计数 |
| 校正收敛 | 每次失败记录 attempt、issues 和草稿 SHA-256；后续提示累积全部历史问题，并禁止来源外数字、百分比、引用和标识符 |
| 交付质量 | Nature 审稿必须含严重级别和至少一段可在当前完整稿件中精确匹配的原文引文，否则继续正常校正 |
| 重复调用 | runtime 已完成预取后明确要求模型直接审查，不再重复拉取同一状态页 |

校正失败仍会诚实地返回 `FAILED`；系统没有把 fallback 或“确定性降级交付”算作稳定成功。

### 14.4 单一 Agent 面板与脏编辑器说明

旧 `AiPanel.vue`、`useAiPanelState.ts`、`useTypewriter.ts` 及对应测试已经删除；正文、LaTeX 和语音入口统一进入全局 `AgentPanel`。本轮净删除约 2,300 行，并删除旧面板 i18n、状态和插入/撤销桥接代码。Mind Map 内部的分析侧栏是不同功能，继续保留。

“脏编辑器”表示 Monaco 内存内容相对磁盘文件有未保存修改。Agent 写盘后若强制刷新该 tab，会覆盖用户尚未保存的内容，因此 runtime 必须停止冲突写入并要求用户先保存或放弃修改。这是数据安全约束，不应删除。本次截图对应会话为 `dirty=false`，实际问题是学术状态定位和答复收敛，不是脏编辑器拦截。

### 14.5 验证边界

- 修复前本地真实回放稳定复现 4 轮新事实漂移和最终 `FAILED`。
- 修复中间态本地真实回放达到 `COMPLETE`，Graph 缺失、Ledger 陈旧和 Reviewer 状态均以成功包络返回，工具错误为 0；该回放同时暴露“没有原文锚点”的交付质量缺口。
- 新增确定性端到端回归：第一次仅返回空泛“审查完成”时触发自动校正，第二次只有在给出严重级别和精确原文锚点后才进入 `COMPLETE`。
- 最后一次同稿、本机模型回放被 Codex 桌面环境的额度审批拒绝，未绕过权限执行；因此不能把新增交付契约描述成已经再次通过同稿真实模型验收。
- 后端全量：`2358 passed, 7 skipped`；其中新增的交付收敛用例已进入全套。
- 前端全量：`824 passed`；TypeScript、Ruff、Prettier、Git whitespace 和生产构建均通过；ESLint 为 `0 errors`，保留 110 条既有 warnings。

这组证据支持“已修复已知确定性失败链并建立回归不变量”，不支持“任何模型、网络和外部服务永远零错误”的不可验证承诺。

## 15. 2026-08-01 用户复核：删除内容层硬门禁，改为提示词约束

### 15.1 修正结论

用户复核后明确拒绝第 14 节采用的运行时内容门禁。该判断成立：对模型答复做启发式数字、引文和语义扫描，会把内容质量问题转化成控制流问题；强制四轮重写、自动学术状态预取和拒绝聊天答复还会额外消耗模型调用并制造停顿。第 14 节的硬门禁方案因此被撤销，不再作为当前架构。

当前边界调整为：

1. 学术事实可靠性、引用规范、Reviewer 查询顺序、陈旧 Ledger 的解释方式和审稿输出格式由系统提示词与技能提示词约束。
2. runtime 不判断某个数字、术语、引文或审稿意见是否“学术正确”，也不因答复缺少严重级别或原文引文而强制重写。
3. runtime 继续负责可确定验证的安全与协议事实：工作区边界、写入审批、dirty editor 冲突、工具参数 schema、工具调用协议、总资源上限、mutation journal 和真实终态。

### 15.2 删除的运行时限制

- 删除 `_UngroundedResponse`、最终答复事实扫描、语义断言扫描、四次自动重写和 `response_grounding_failed`。
- 删除 Nature 审稿的强制 Graph/Ledger/Reviewer 预取；模型根据 `nature_reviewer` 提示词自行选择和调用这些工具。
- 删除“必须包含严重级别和精确原文引文才能交付”的 runtime 判定；这些要求只保留在技能提示词。
- 删除学术文件 mutation 的数字/引文扫描、`academic_evidence_required`、`evidence_refs` 工具参数和证据 ID 重绑定逻辑。
- 删除 pending mutation 后强制丢弃两次模型答复的机制。若真实文件操作仍未完成，首个答复可以正常显示，但 session 诚实标记为 `PARTIAL` 并携带 pending action，不再循环重写。
- 删除“相同工具参数调用若干次即终止”的断路器。真实审稿会对 Ledger/Reviewer 分页或复读；重复次数不是可靠的失败判据。只保留整个会话的工具调用总预算和研究工具预算。

### 15.3 保留的稳定性边界

以下机制不是内容审查限制，不能用提示词可靠替代，因此继续保留：

- 路径逃逸阻止和 workspace grant；
- 写入、命令和外部副作用审批；
- Monaco 未保存内容与磁盘版本冲突保护；
- 工具 JSON/schema 校验及 DSML 等错误工具协议隔离；
- 工具/模型调用总预算和 Abort；
- 原子写入、分块完成标志、checkpoint、Undo 和 pending action 持久化。

### 15.4 新行为验证

- 带新增数值的学术正文修改不再被 runtime 事实扫描阻止，可进入正常审批/写入/checkpoint 流程。
- 启用学术技能后，模型的第一份最终答复直接交付，不再由 runtime 静默丢弃或自动改写。
- 文件 mutation 真实失败后，如果模型选择回答，runtime 只交付一次并标记 `PARTIAL`，不再强制追加两轮模型调用。
- 提示词继续要求不编造事实、使用当前稿件路径查询学术状态、区分陈旧或不可用状态，并输出可操作审稿意见。
- 本机 `qwen3:8b` 改用 Ollama 原生 `/api/chat`，每次请求显式设置 `num_ctx=32768`；不再经兼容接口以 4096 上下文运行。原生 provider 的工具调用、流式文本、thinking 和 usage 均有协议回归。
- DeepSeek Flash 真实长稿 Nature 审查完成 9 次工具调用，9 次均成功：按当前 `doc_id` 读取 Graph、Ledger 与 Reviewer 并完成分页，最终正常 `response` + `done`，无 stop code、无 pending action。
- DeepSeek Flash 真实跨轮流程先两次读取当前稿件并总结，第二轮沿用同一 session 继续回答；两轮均正常完成且没有工具错误。
- 工具切换产生的空 `tool_turn` 控制事件仍负责清除 provisional text，但前端不再把它加入可见 warning 时间线；真实网络重试告警保持可见。
- Agent V2 全套：`802 passed`。
- 后端全量：`2349 passed, 7 skipped`。
- 前端全量：`825 passed`；TypeScript、Ruff、Prettier、ESLint（`0 errors`）和生产构建通过。

该方案消除了启发式内容门禁和重复调用次数门禁导致的确定性停顿；相应代价是 runtime 不再为模型内容真实性兜底，内容质量依赖所选模型、提示词和实际工具证据。这是本轮用户明确选择的架构取舍。第 13、14 节记录的是问题发现过程，其中的强制预取、内容扫描、重复调用断路和旧测试计数均已由本节取代。

## 16. 2026-08-01 长任务复核：固定调用上限改为进展租约

### 16.1 结论与现场证据

最新真实会话 `sess_0fa4615ac1344576ba99fd15cefea948` 证明第 15 节保留“工具/模型调用总预算”的结论仍不可靠：该 session 聚合为 5 个 turn、81 次成功工具结果、0 次工具错误和 33 次 mutation；其中“全部给我改好”一轮恰好完成 32 次成功工具调用后没有形成最终答复，用户点击继续后又用 20 次成功工具调用完成。停止点与 `max_model_calls=32` 精确重合，并非工具失败、工作区冲突或 64 次工具上限。

DeepSeek Flash 在该流程中通常每次模型响应只选择一个工具。固定模型调用次数因此测量的是 provider 的工具编排粒度，而不是任务是否仍在推进；同一任务若模型一次批量返回多个工具就更不容易触顶。这种限制会把连续成功误判为资源耗尽，要求用户反复点击继续。

### 16.2 当前运行机制

普通 Agent turn 已删除以下固定次数：

- `max_steps`；
- `max_tool_calls` 与 `soft_tool_calls`；
- `max_model_calls`；
- `max_mutation_attempts`；
- 独立 research tool 次数和 soft research 次数。

替代机制是可续租的进展窗口：

1. 每个成功工具结果对 `tool_name + 规范化输入 + 输出` 计算观察签名；出现新签名即认定取得新结果，清零连续停滞次数并续租活动执行时间。
2. 默认只有连续 12 次工具调用没有新结果才进入 `PARTIAL / no_progress_stall`。相同成功观察、`no_change` 和失败结果均计入连续停滞；中间任一新成功观察会清零。
3. 达到一半窗口时只通过系统提示要求模型换查询、路径、分页或编辑策略；不阻止仍在产生新结果的检索、研究、写入和验证。
4. 仍保留用户 Abort、连续 5 次工具错误、600 秒无新成功观察的活动租约，以及选区编辑的 4 次原子操作边界。后者只保护一个 Monaco 选区，不限制普通长任务。
5. 配置、状态接口、i18n 和配置校验已移除废弃次数字段，只暴露 `max_active_seconds` 与 `max_stalled_tool_calls`。

全量回归还发现一个看门狗旁路：重复提交已经成功应用过的相同 mutation 会从幂等分支提前返回，最初没有记录 `no_change`，导致旧集成用例无限循环。该分支现已纳入停滞窗口；对应超时用例恢复为亚秒级完成。

### 16.3 同轮稳定性修复

- 成功 mutation 后，runtime 会把新的文件哈希推进为本轮 clean editor 基线；同一 turn 的第二次正常写入不再被初始编辑器哈希误判为外部修改。真正的外部磁盘变化仍会触发版本冲突。
- 模型在工具调用前输出的普通 planning 文本先缓冲；若该轮随后出现工具调用，只在折叠思考区显示确定性的工具阶段摘要，不再把规划过程作为正文逐字展示。无工具的最终答复仍正常输出。

### 16.4 验证证据

- 进展回归：40 个不同的单工具模型轮次自动运行到第 41 次模型调用并 `COMPLETE`，越过旧 32 次边界。
- 停滞回归：相同输入和相同结果在配置窗口内停止并持久化 `no_progress_stall`；批量调用停止时仍为每个 tool use 补齐 tool result，保持 session 协议可恢复。
- 活动租约回归：三个累计耗时超过单个租约、但逐次产生新观察的慢工具均完成；单个无结果操作超过租约仍被取消并保存 `PARTIAL`。
- 真实 DeepSeek Flash 隔离验证：`deepseek-v4-flash` 连续完成 35 个依赖前一步不透明令牌的工具步骤，共 36 次模型调用、35 次成功工具结果、0 工具错误、0 runtime error；终态 `COMPLETE`、`stop_code` 为空，最终答复为 `LIVEQA 35 COMPLETE`。隔离脚本和 33 KB session 随后已删除。
- Agent V2 全套：`805 passed`。
- 后端全量：`2347 passed, 7 skipped`。
- 前端全套：`825 passed`；TypeScript、测试 TypeScript、Prettier 和 ESLint 均通过，ESLint 为 0 error、110 条既有 warning；生产构建通过。

本节取代第 15 节关于“保留整个会话工具/模型调用总预算”的结论。当前机制不承诺模型或网络永远不出错；它保证的是：只要工具持续产生新的成功结果，普通 Agent turn 不会再因为累计调用次数达到某个常数而被切断。
