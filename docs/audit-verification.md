...
| **F3** checkpoint 决策 stub + 死 UI | TRUE | `respondCheckpoint`（原 L597）仅清空状态、`_decision` 未用；`AgentCheckpointCard.vue`（50 行）全 `src` **零引用**；router.py **无** checkpoint decide 端点；`respondCheckpoint` 在任何 .vue/.ts 中**零调用** |
...
| F3 | 删除死组件 `AgentCheckpointCard.vue`；移除未使用的 `respondCheckpoint` stub（函数体 + return 导出） | `src/components/AgentCheckpointCard.vue`（删）、`src/composables/useAgentChat.ts` | 全 `src` 无残留引用；`useAgentChat.test.ts` 20 passed |
...
- 本会话的所有改动**未自动 git commit**（涉及用户 WIP 分支，需用户决策）。
- 未删除 180+ 未跟踪截图/文档（属用户资产，需用户确认）。
- **建议**：用户在方便时把本会话改动与现有 WIP 分别原子提交；截图归入 `artifacts/` + gitignore；提交 `pnpm-lock.yaml`。
