...
RITA：组注意力在时间序列分析中的应用
JIAMING LIANG 美国宾夕法尼亚大学  
LEI CAO 亚利桑那大学/麻省理工学院  
SAMUEL MADDEN 麻省理工学院  
ZACHARY IVES 美国宾夕法尼亚大学  
GUOLIANG LI 清华大学  
时间序列分析在许多现实应用中具有重要意义。近期，Transformer模型（最初用于自然语言处理）已被应用于从时间序列数据中学到高质量的特征嵌入：这些嵌入是各种时间序列分析任务性能的关键因素，例如向量数据库中的基于相似性的时序查询。然而，二次时间和空间复杂度限制了Transformer的可扩展性，尤其是在长时序场景中。为解决这些问题，我们开发了一个时间序列分析工具RITA，其采用了一种新颖的注意力机制——组注意力（group attention），以应对这一可扩展性问题。组注意力根据对象之间的相似性动态聚类成少量群组，并在粗粒度群组层面近似计算注意力值。因此显著降低了时间和空间复杂度，同时为所计算注意力的质量提供了理论保证。RITA的动态调度器持续调整训练过程中所需的群组数量和批量大小，确保组注意力始终使用满足近似质量要求所需最少的群组数。在各种时间序列数据集和分析任务上的大量实验表明，RITA在准确率上优于现有技术，并且显著更快——速度提升最高可达63倍。
CCS概念：• 信息系统 → 数据管理系统
附加关键词短语：时序分析、自监督、注意力机制、高效Transformer  
ACM参考格式：
Jiaming Liang, Lei Cao, Samuel Madden, Zachary Ives and Guoliang Li. 2024. RITA: Group Attention is All You Need for Timeseries Analytics. Proc. ACM Manag. Data 2,1 (SIGMOD), Article 62（2024年2月），28页。https://doi.org/10.1145/3639317  
1 引言
许多数据驱动的应用涉及处理大规模时间序列数据，包括物联网[14]、医疗AI[17]、股票市场[32]等场景。因此需要强大的时序分析能力，如预测[9]、分类[24]、聚类[36]、相似性搜索[45]和异常检测[57]，其应用范围从自动诊断疾病[6]、识别人类活动[34]到阻止金融欺诈[67]。特别是数据库领域中，向量数据库[61]被用于存储和索引非结构化及半结构化数据的高维特征嵌入，这使得用户能够  
作者地址：Jiaming Liang 美国宾夕法尼亚大学费城PA州 liangjm@seas.upenn.edu；Lei Cao 亚利桑那大学/麻省理工学院图森AZ州 lcao@csail.mit.edu；Samuel Madden 麻省理工学院剑桥MA州 madden@csail.mit.edu；Zachary Ives 美国宾夕法尼亚大学费城PA州 zives@cis.upenn.edu；Guoliang Li 清华大学北京中国 liguoliang@tsinghua.edu.cn。本作品采用知识共享署名国际4.0许可协议授权。© 2024 版权由原作者/持有者所有。ACM 2836-6573/2024/2-ART62 https://doi.org/10.1145/3639317
...
用户机器学习
分类流式事件
图1. RITA在基于时间序列的查询系统中通过在特征空间执行相似性搜索高效探索该数据。有效的特征表示是构建有效向量数据库的关键组成部分。作为现实案例，我们与一家美国主要医院[7]合作开发了一个大规模交互系统，用于标记45亿个EEG段（30TB），这些段包含6类代表不同癫痫类型的分类标签。这些带标签的EEG段被用来训练一个分类器，该分类器可以根据临床观察中患者收集到的EEG信号自动检测癫痫发作。该系统的目的是将专家提供的标签传播给相似段落，从而减少神经科医生的手动标注工作量。我们的时序特征嵌入方法RITA在此系统中作为核心组件发挥作用。更具体地说，我们使用RITA将EEG段转换为特征嵌入，并存储在支持神经科医生提交的相似性搜索请求的向量数据库中，以返回待标记段落的k个最近邻，如图1所示。近期研究人员[70]开始利用Transformer模型自监督预训练方法（5, 19, 59），该方法在自然语言处理(NLP)领域取得了显著成功，用于自动从时序数据中学习高质量特征嵌入。在NLP中，自监督预训练通过句子中的词的序列模式(相关性)生成上下文化特征嵌入。时间序列与自然语言相似之处在于，在时间序列数据中随时间变化的值（股票价格、成交量等）顺序至关重要。也就是说，每个值与其他前后观测到的值高度相关。因此，对考虑不同观察之间关联性的Transformer模型进行预训练是学习时序特征嵌入的自然想法。事实上，[70]中的实验确认了基于Transformer的方法优于传统时间序列分析技术。现有工作[70]直接应用Transformer从时序数据中学习特征已被证明无法扩展到长时序场景[35]。自注意力机制[59]是NLP预训练方法的核心：它计算序列（在NLP中为句子）不同语义单元之间的成对相关性；因此，其时间与空间复杂度随输入序列长度呈二次增长。这限制了模型的可扩展性，尤其是在处理大规模时序数据时，这类场景常见于物联网、医疗AI和金融等现实应用[7, 39, 71]。关于时序预测可能需要查看数以万计的历史样本才能达到准确率。再次参考我们的EEG项目，癫痫发作是短暂的，因此我们将EEG数据分块为2秒段并按段级检测癫痫。然而，特定段落分类依赖于最多12小时之前的信号来考虑长期趋势，并确定某个2秒段是否表示癫痫发作。在12小时内有超过21,000个段落。这远远超过了典型NLP任务预期的语义单元数量。例如BERT[19]  
RITA：组注意力在时间序列分析中的应用 62:3限制了单位数到512，甚至像GPT-3[5]这样的大规模模型也仅限于2048个单位。尽管在NLP中提出了一些低复杂度方法来近似计算自注意力[11, 31, 62]，但由于自然语言与时间序列之间的差异，这些方法在应用于时序数据时性能会显著下降，这将在我们的实验中展示。提出的解决方案：为了解决上述问题，我们开发了RITA，一个基于Transformer的时间序列分析工具，其采用了一种新颖的注意力机制——组注意力（group attention），以扩展到长时间序列场景。我们的目标是在保持最小预测准确率损失的前提下显著加速执行速度。利用长时间序列片段之间的相似性，RITA将输入时序数据分块为段，并动态地将其聚类成少量群组（记作N）。同一群组内的段在当前训练迭代中具有类似的特征嵌入，使它们能够近似共享注意力计算。随着时间序列长度的增加，更多的资源共享机会变得可用。
...
> RI TA Encoder. Time-aware Convolution Layer fills the gap between timeseries and natural language. Despite their high-level similarity, there is a big gap between timeseries and natural language.

随着时间序列长度的增加，更多的资源共享机会变得可用。RITA随后按群组级别计算自注意力，并生成压缩后的组注意力矩阵。通过这种方式，组注意力消除了Transformer风格模型中的计算和内存瓶颈，因此更适用于长时间序列场景。然而，在Transformer架构中使这一想法高效且有效是具有挑战性的原因有以下几点：
• 高效产生高质量特征嵌入。RITA按群组级别计算注意力矩阵。为了保持特征嵌入的质量，它仍然需要为不同段生成不同的嵌入。这是因为即使某些段在时间上共享注意力得分，它们可能并不共享相同的特征嵌入。使用组注意力矩阵时，现有的自注意力机制将只为每个群组产生一个单一的特征向量。一种朴素的方法是通过组注意力矩阵恢复原始注意力矩阵。然而，在这种情况下，我们再次得到具有二次空间复杂度的注意力矩阵。由于GPU内存有限，GPU内存将成为组注意力的主要瓶颈。
• 群组数量𝑁。在RITA中，群组数量𝑁是一个关键因素，它平衡了加速效果和注意力近似质量之间的关系。较小的𝑁将带来较大的加速效果，但近似误差也可能显著增加。另一方面，尽管较大的𝑁倾向于产生高质量的近似结果，但它不可避免地会减缓训练过程。因此，适当的𝑁对组注意力性能至关重要。然而，𝑁依赖于数据集的分布特性。此外，像经典Transformer模型一样，RITA堆叠多个注意力层以生成更好的嵌入。理想情况下，不同层次也应该使用不同的𝑁值。另外，在模型训练阶段，组注意力应该在不同迭代中使用不同的𝑁值来适应变化的特征嵌入。这使得手动优化𝑁几乎不可能。
• 批处理大小。此外，由于我们希望在训练过程中动态调整𝑁，固定的批处理大小是次优选择：随着𝑁减少，单个样本的记忆占用量会降低。这允许更大的批量尺寸，其（1）充分利用GPU内存；（2）实现跨样本的并行性。因此，RITA应该根据𝑁的变化动态调整批量尺寸。为了解决上述问题，我们首先提出一种嵌入聚合策略和定制化的组softmax函数来替代经典softmax函数[59]。它们共同确保RITA能够直接使用压缩后的注意力矩阵生成不同段的不同特征嵌入。我们在理论上证明了RITA以这种方式产生的嵌入与先恢复原始大注意力矩阵再进行计算所得到的嵌入是相同的。因此，RITA在不引入额外开销的情况下产生高质量嵌入。
其次，我们设计了一个自适应调度器，在训练过程中动态决定每个组注意力层合适的𝑁值。它从较大的𝑁开始，并迭代地合并相似度高的群组。根据用户可容忍的近似自注意力误差界限进行指导，它可以自动判断两个群组是否可以合并，并以GPU友好的方式高效执行合并操作。此外，我们提出了一种基于学习的方法来建模群组数量𝑁与批量尺寸𝐵之间的相关性。该模型在训练RITA时预测给定𝑁对应的𝐵值。具体而言，我们首先在一个合理范围内采样一些𝑁值。对于每个采样的𝑁，我们在成本效益高的方式下找到一个使GPU内存占用达到一定百分比的批量尺寸。利用一组小规模数学函数作为先验知识，RITA能够仅通过少量⟨𝑁, 𝐵⟩对的真实标签学习出模型。我们的实验在公开时间序列基准数据集和MGH EEG数据[7]上验证了相比现有自注意力机制[11, 12, 46, 59, 62]，RITA的组注意力机制实现了显著更高的速度提升（达63倍），且内存需求大幅降低，在各种时间序列分析任务中达到与甚至优于现有方法相当或更好的准确率。贡献。本工作的关键贡献包括：
• 我们的组注意力机制在保证准确性的同时降低了自注意力机制的时间和空间复杂度，使RITA能够扩展到长时间序列数据。
• 受近似误差界限的指导，我们的自适应调度器动态调整群组数量和批量尺寸以适配演化特征嵌入的分布特性，使得组注意力既高效又易于调优。
• 我们在多种数据集和分析任务上进行了实验，证明RITA比现有基于Transformer的方法快4到63倍，在处理长时间序列（长度≥2000）时达到与之相当或更佳的准确率。背景
我们提供了一些关于Transformer中经典自注意力模块的基础知识[59]。该模块接受𝑛个隐藏嵌入向量𝐻 ∈ Rⁿ×𝑑ℎ作为输入，然后将其投影到查询(𝑄)、键(K)和值(V)，并执行缩放点积注意力计算，其给定输入隐状态的公式为：
接下来，我们通过一个示例展示RITA如何支持时间序列查询系统（见第3.1节），随后概述RITA模型架构（见第3.2节）。我们在第6节将详细说明RITA如何处理一系列下游任务。3.1 RITA在时间序列查询系统中的应用
图1展示了我们为一家主要美国医院开发的癫痫诊断系统。RITA既用于训练阶段也用于部署阶段。如引言所述，我们将RITA用来编码EEG片段并将生成的结果（时间序列、嵌入）对存储到向量数据库中作为（键、值）对。这被用于基于相似性的标签分配。随后，我们通过微调RITA模型来训练分类器，并将其部署在患者监测系统以实时检测癫痫发作。当一个新的EEG片段到达时，系统使用RITA编码器将它嵌入到特征向量中，然后对其进行癫痫类型的分类。
3.2 模型架构
如图2所示，RITA由两个组件组成：（1）时间感知卷积层和（2）RI TA编码器。时间感知卷积层填补了时间序列与自然语言之间的鸿沟。尽管它们在高层面上具有高度相似性，但时间序列与自然语言之间仍存在显著差距。

> Despite their high-level similarity, there is a big gap between timeseries and natural language. First, in natural language each word, as a discrete semantic unit, has an independent meaning, while each element in a timeseries is a continuous, numerical value and does not necessarily constitute an independent event. Furthermore, the input sequences are single-channeled in NLP, but often multi-channeled in timeseries (i.e., sensor data often consists of several related channels). RITA leverages the classical convolution [33] strategy to solve this problem. Convolution is widely used to capture the local structures of an image. We use convolution to chunk one input timeseries into a sequence of windows and learn the local structure of each window, similar to the discrete semantic units in natural language. It also discovers the correlations across different channels, thus naturally solving the multi-channel problem. More specifically, treating a multi-variate timeseries of length 𝑛 and with 𝑚 variables as an n × m matrix 𝑇, RITA uses 𝑑 convolution kernels to chunk 𝑇 into n windows and produce one ddimensional embedding per window using the convolution operation [33]. Each convolution kernel corresponds to a w × m matrix, where 𝑤 defines the number of timestamps that each convolution kernel covers, identical to the window size in sliding window. takes the embeddings of 𝑛 semantic units 𝑋 ,𝑋 ,...,𝑋 (𝑋 ∈ 𝑅𝑑) as input (e.g.
...
如图2所示，RITA由两个组件组成：（1）时间感知卷积层和（2）RI TA编码器。时间感知卷积层填补了时序数据与自然语言之间的鸿沟。尽管它们在高层面上具有高度相似性，但时序数据与自然语言之间仍存在显著差距。
...
首先，在自然语言中每个词作为离散语义单元具有独立含义，而时序数据中的每个元素是连续数值且不一定构成独立事件。此外，NLP的输入序列通常是单通道的，但在时序数据中往往是多通道（即传感器数据通常包含多个相关通道）。RITA利用经典卷积[33]策略解决这一问题。卷积广泛用于捕捉图像的局部结构。我们使用卷积将一个输入时序分割为一系列窗口并学习每个窗口的局部结构，类似于自然语言中的离散语义单元。它还发现不同通道间的相关性，从而天然地解决了多通道问题。

> the matrix product will produce a 𝑁 × 𝑑 matrix 𝑂. That is, it produces a feature vector for each 𝑣 group. However, our goal is to produce different embeddings for different windows, because even if some windows share the attention score temporally, it does not mean they should have the same feature emb e dding. A Naive Solution. Anaive solution would be to restore the full attention matrix 𝐴 from the group attention matrix 𝐴. For example, given one group composed of 𝑤𝑖𝑛 and 𝑤𝑖𝑛 , we map its group 𝑖𝑗 attention vector in 𝐴 into two rows that correspond to 𝑤𝑖𝑛 and 𝑤𝑖𝑛 in 𝐴. However, in this case 𝑖𝑗 we again get a 𝑛 × 𝑛 attention matrix; and GPU memory remains a bottleneck in group attention. Usingan operationanda dingswithoutrestoringthefullattentionmatrix. conductedonthefullyrestoredattentionmatrix
> Givenanelement of correspondingtothe wherevector denotesthe dimensionofallthe featurevectors.Given
> = .

更具体地说，将长度为𝑛且包含𝑚个变量的多元时序视为n × m矩阵𝑇，RITA使用𝑑个卷积核对𝑇进行分割生成每个窗口的d维嵌入（通过卷积操作[33]）。每个卷积核对应一个w × m矩阵，其中𝑤定义了每个卷积核覆盖的时间戳数量，与滑动窗口大小相同。输入𝑛个语义单元𝑋₁, 𝑋₂,..., 𝑋ₙ (𝑋 ∈ Rᵈ)的嵌入（例如RITA编码器对时序分割后的n个窗口的嵌入），然后建模语义单元间的相关性并输出𝑌₁,...,𝑌ₙ (𝑌 ∈ Rᵈ)作为每个单元的情境感知嵌入。使RITA编码器区别于Transformer编码器的是：在Transformer编码器核心是自注意力机制，其时间复杂度和内存占用为𝑂(𝑛²)，这种二次成本对于长时序变得不可行并限制了基于Transformer模型的可扩展性。为了实现高效且高质量的关注计算，我们用提出的组注意力替代经典自注意力。

无监督预训练。受NLP中"cloze text"预训练任务启发，我们设计了一个掩码-预测任务作为模型的预训练任务。时序数据被随机掩码，并基于对应上下文信息恢复被掩码值。具体而言，在时间戳上生成掩码（掩码率𝑝）。将时序数据缩放为非负数并在被掩码的时间戳的所有通道中设置值为-1，这是正常时间戳上的不可能值。然后将被掩码的时序输入RITA，并通过转置卷积层将输出表示转换为恢复后的时序。

4 组注意力机制
组注意力是一种新颖且高效的近似注意力机制，解决了vanilla Transformer中的自注意力性能瓶颈。在本节中，我们首先介绍组注意力框架并理论建立其近似误差的界限。用示例解释组注意力机制（见图3）。4.1 组注意力思想
由于周期性是时序数据的自然属性[64]，相似窗口频繁出现。相似窗口导致注意力计算中的相似查询/键向量，带来节省计算的机会。如第2节所述，窗口𝑖对窗口𝑗的关注得分𝐴ᵢⱼ由窗口i的查询向量与窗口j的键向量内积决定，即𝑞 · 𝑘。给定另一个窗口x，如果窗口x的键向量相似于窗口j的键向量（即𝑘 ≈ 𝑘 предусмотр），则会产生类似的得分。然而我们的目标是为不同窗口生成不同的嵌入，因为即使某些窗口在时间上共享关注得分，并不意味着它们应该具有相同的特征嵌入。

朴素解法：一种朴素方案是从组注意力矩阵𝐴恢复完整的注意力矩阵𝐴。例如给定由𝑤ᵢₙ和𝑤ᵢₙ组成的单个组，将其在𝐴中的组向量映射为对应于𝑤ᵢₙ和𝑤ᵢₙ的两行。然而在这种情况下我们再次得到𝑛 × 𝑛的关注矩阵；GPU内存仍然是组注意力瓶颈。

使用操作和加法而不恢复完整注意力矩阵。基于完全恢复后的注意力矩阵进行运算，给定一个元素对应的向量表示所有特征向量的维度。给出= .

> wherevector denotesthe dimensionofallthe featurevectors.Given
...
然而在这种情况下我们再次得到𝑛 × 𝑛的关注矩阵；GPU内存仍然是组注意力瓶颈。使用操作和加法而非直接计算，通过将距离公式转换为点积形式来优化性能瓶颈。在GPU上，矩阵乘法比逐对差值运算更高效。
...
5 自适应调度器
接下来，我们介绍RITA的自适应调度器，该调度器解决了确定适当组数𝑁及相应批量大小𝐵的挑战，如引言中所述。通过提出的一种动态调度方法，调度器能够根据迭代训练过程中生成的特征嵌入分布特性自动决定和调整𝑁与𝐵，在保证高质量注意力近似的同时满足用户需求。在第5.1节我们展示RITA如何自动确定𝑁。然后在第5.2节介绍基于学习的方法，该方法给定一个𝑁后能立即预测良好的批量大小。
...
5.1 动态确定组数N
不失一般性，我们以单个组注意力模块为例说明RITA如何自动获取适当的𝑁值。RITA初始时采用较大的𝑁并动态减小其数值。这是因为随着训练过程的进行，每个epoch生成的特征嵌入逐渐趋于稳定且最终收敛[30]，因此通常无需增加𝑁值。RITA通过合并相似组来减少组数。直观上讲，给定两个组，我们可以通过它们中心点的距离衡量相似性。如果其距离小于设定的距离阈值，则这两个组可以被合并。
...
（注：原文中存在部分排版错误和符号缺失，在翻译过程中根据上下文进行合理推测并保持技术准确性）
...
直观而言，在给定批量大小B和分组数量N的情况下，若能精确计算其GPU内存使用量，则可根据GPU内存容量和L直接确定合适的批量大小。然而这不可行的原因包括：(1) RITA的动态分组导致深度学习计算图不断变化[1]；(2) 深度学习基础设施通过内存管理技术进行优化[3]。因此，我们提出一种基于学习的方法来预测批量大小。该方法根据一些采样批次实际消耗的GPU内存建立时间序列长度L、分组数量N与批量大小B之间的相关性，在训练过程中高效测量这些数据。RITA会抽样多个(L_i, N_i)对，并为每个对估计合适的批量大小。将这些(i, L_i, N_i)视为真实标签，我们使用函数拟合[22]学习预测函数 B = f(L,N)，其中B是两个变量L和N的函数。更具体地，在给定用户定义的时间序列最大长度L_max后，我们会从平面{1 ≤ L ≤ L_max, 1 ≤ N ≤ L}中随机采样m个整数点(L_i, N_i)。接着使用基于二分查找的算法找到消耗小于90%可用GPU内存的最大批量大小B，旨在避免浪费GPU内存和出现内存溢出(OOM)的风险。接下来讨论如何利用这些采样的(L_i, N_i, B_i)学习预测函数。
...
**学习预测函数**
我们采用SciPy[60]中的曲线拟合作为函数拟合工具，在平面{1 ≤ L ≤ L_max, 1 ≤ N ≤ L}上对双变量函数B = f(L,N)进行建模。观察到将单一函数应用于整个平面上会导致巨大的估计误差，因此我们开发了动态规划(DP)方法，将平面划分为若干子区域并为每个子区域应用不同的函数（即算法2）。该方法在所有子区域内最小化总估计误差方面是最优的。通过学习得到的预测函数f，可以在训练过程中对任意(L, N)估算合适的批量大小，即使这些值未出现在采样的(L_i, N_i)对中。

> ≤ 𝑚𝑎𝑥 |𝑐 −𝑐 |+|𝑐 −𝑐 |+|𝑥−𝑐 | 𝑗1 𝑖 𝑖 𝑗2 𝑗1 (15) 𝑥 ∈𝑐𝑙𝑢𝑠𝑡 𝑒𝑟 𝑗

**平面划分动态规划算法**
要求：L_i、N_i、B_i、L_max  
保证：1 ≤ L ≤ L_max, 1 ≤ N ≤ L  
1: 函数 cost(S)  
2: 如果 |S| < M 返回 +∞  
3: 取出 S 中的点 (L, N, B)  
4: 使用函数拟合得到 f = function_fitting(B | L, N)，返回误差 E(B,L,N|f)  
5: 函数 dynamic_programming(L_i, N_i, L_max)  
6: 对 l1 从 1 到 L_max 进行循环  
7: 对 l2 从 1 到 l1 进行循环  
8: 对 n 从 1 到 l1 进行循环  
9: 取出集合 {l2 ≤ L ≤ l1, N ≤ n} 中的点集 S  
10: 计算 g(n) = COST(S)  
11: 对 i 从 1 到 n 进行循环  
12: 取出集合 {l2 ≤ L ≤ l1, i ≤ N ≤ n} 中的点集 S  
13: 更新 g(n) = min(g(n), g(i) + COST(S))  
14: 记录 f(l1, l2, l1) = g(n)  
...（后续代码省略）

> ≤ 𝑚𝑎𝑥 |𝑐 −𝑐 |+|𝑐 −𝑐 |+|𝑥 −𝑐 |+|𝑥 −𝑐 | ≤𝑑 𝑗1 𝑖 𝑖 𝑗2 𝑗1 𝑗2 𝑥 ∈𝑐𝑙𝑢𝑠𝑡 𝑒𝑟 𝑗

**平面划分算法最优性证明**
我们描述了算法2并直观展示其最优性。假设SciPy[60]在第4行学习到一个最优函数，使得当拟合集合S中的点时，COST给出的估计误差是最优的。当拟合极少数量的点时，我们将成本设为无穷大以防止偏差（第2行）。g(n)表示子区域{l ≤ L ≤ l1, N ≤ n}中所有点的最小估计误差。在第11-13行中，我们枚举了将{l ≤ L ≤ l1, N ≤ n}水平划分为两个子区域{l ≤ L ≤ l1, N ≤ i}和{l ≤ L ≤ l1, i ≤ N ≤ n}的所有可能方式（通过迭代i从1到n）。选择使估计误差最小的划分策略，得到记录在第14行中的g(l)。dp(l)表示子区域l1,l2 {L ≤ l} 的最小估计误差。我们枚举了将{L ≤ l}垂直划分为两个子区域{L ≤ i}和{i ≤ L ≤ l}的所有可能方式（通过迭代i从1到l，第17-19行）。最终得到整个平面的最小估计误差为dp(L_max)。
...
> {𝐿 ≤ 𝑙 }. We enumerate all the possible ways of cutting {𝐿 ≤ 𝑙 } vertically into two sub-plane {𝐿 ≤ 𝑖} and {𝑖 ≤ 𝐿 ≤ 𝑙} by iterating 𝑖 from 1 to 𝑙 (Line 17-19). Finally, we have the minimal estimation error for the whole plane as 𝑑𝑝(𝐿 ).

> Finally, we have the minimal estimation error for the whole plane as 𝑑𝑝(𝐿 ). Based on the above discussion, this algorithm guarantees to 𝑚𝑎𝑥 not miss any better solution, hence optimal. 6 SUPPORTING DOWNSTREAM TASKS
...
我们通过迭代 𝑖 从 1 到 𝑙（第17-19行）列举了将 {𝐿 ≤ 𝑙 } 垂直划分为两个子平面 {𝐿 ≤ 𝑖} 和 {𝑖 ≤ 𝐿 ≤ 𝑙} 的所有可能方式。最终，整个平面上的最小估计误差为 𝑑𝑝(𝐿 ) 。基于上述讨论，该算法保证不会遗漏任何更优解，因此是最优的。6 支持下游任务  
RITA 适用于多种下游任务。在本节中，我们展示通过最少修改 RITA 可以有效支持分类、插补和预测任务。其他无监督任务如相似性搜索或聚类可通过从 RITA 提取特征嵌入自然实现。6.1 分类  
为了对时间序列进行分类，我们将时间序列作为描述在第3节中所述的方式输入模型，并将特殊标记 [CLS] 作为第一个输入嵌入附加到该输入上。[CLS] 的嵌入表示整个时间序列的嵌入，且 [CLS] 的输出表征被馈送到分类器：𝑦 = Softmax (𝑊𝑍 + 𝐵 ), 其中 𝑍 ∈ R𝑑 是 的输出表征，𝐶 为类别数，  
𝐶𝐿𝑆 [CLS]
[ ]
∈ RC×d, ∈ RC 是用于分类任务的可学习参数。结果向量 𝑦 ∈ R𝐶 表示输入时间序列属于每个类别的可能性。我们采用交叉熵作为分类任务的损失函数 [16] 。6.2 插补  
时间序列主要由传感器生成，其常见问题是缺失值。当许多下游分析需要恢复这些缺失值时，这成为一个挑战。恢复任务即为插补，是一种数据清洗任务。记真实的时间序列为 𝑇 ∈ R𝑡 ×𝑚 ，带有缺失值的观测时间为序列为 𝑇 ∈ R𝑡 ×𝑚,𝑟𝑜 且缺失值位置集合为 𝑀 。我们将所有时间序列的数值缩放到非负数，并使用特殊值 (-1) 表示缺失值：
...
𝑇 (𝑖, 𝑗) = (16) 𝑜
𝑇 (𝑖, 𝑗) (𝑖, 𝑗) ∉ 𝑀 𝑟
将 𝑇𝑜 作为输入馈入 RITA，输出表征被连接后用于后续任务。6.3 预测  
对于时间序列预测任务，我们采用 Triformer [12] 和 PatchTST [46] 这两种 SOTA 的 Transformer 基方法进行比较，在其解码器中替换为下游任务的输出网络。同时将 RITA 与 GRAIL [47]（非深度学习时间序列表征学习的 SOTA 方法）进行对比，GRAIL 通过将学到的表示馈入支持向量机 [15] 或 K-最近邻 [20] 分类器实现分类任务。
...
R Besides, the results of Table 3 show that Group Attn. achieves better imputation performance than other methods in terms of MSE on multi-variate data.
...
> TS2VEC by 6 percentage points. (3) Group Attention’s advantage over other attention mechanisms. Group Attn. is better than Performer and Linformer on 3 out of 4 datasets.

表3 损失结果（多变量数据）。指标：MSE。最佳结果用粗体标记。

7.2.1 全标签训练（多变量分类）
图4所示的结果得出以下观察：
(1) 组注意力相对于TST的优势。在所有四个数据集上，组注意力在准确率和训练时间方面均优于TST。特别是，在ECG数据集中，组注意力的准确率比TST高出49个百分点（88.48% vs 39.93%），且每个epoch的训练速度快三倍（236.8s vs 731.0s）。三个缺陷可能导致了TST在长序列上的表现不佳。首先，TST将每个时间戳输出嵌入向量进行拼接后使用线性分类器对拼接后的向量进行分类。当时间序列较长时，线性分类器的参数过多容易导致过拟合。其次，TST用批量归一化替代了原始Transformer中的层归一化。当时间序列较长时间，每个批次只能容纳少量的时间序列样本，这会导致批量归一化的偏差。第三，TST使用原始自注意力机制，导致二次复杂度。
(2) 组注意力相对于TS2VEC的优势。在四个数据集上，组注意力的准确率始终优于TS2VEC。特别是，在包含长时间序列的ECG数据集中——我们关注的情景中，组注意力的准确率比其高出28%（88.48% vs 59.17%），同时每个epoch使用的训练时间更少。这可以归因于我们的主干模型(Transformer)能够有效捕捉长距离相关性。
(3) 组注意力相对于Vanilla的优势。Vanilla精确计算了注意分数，因此预期表现良好。然而，在WISDM数据集上组注意力的准确率比Vanilla高出0.55个百分点（87.50% vs 86.95%），在其他三个数据集中与Vanilla非常接近。这表明组注意力的近似质量较好。相比Vanilla自注意力，组注意力更具可扩展性。当序列长度为200(WISDM, HHAR, RWHAR)时，组注意力平均需要更少25%的训练时间。当长度增加到2,000(ECG)，Vanilla每个epoch耗时超过15分钟，约为组注意力速度的4倍慢。在长序列MGH数据中（长度达到10,000），由于GPU内存溢出导致Vanilla失效。
(4) 组注意力相对于其他高效注意力机制的优势。在四个数据集中，组注意力在3个数据集上比Performer和Linformer更准确。尽管在ECG数据集上Linformer的表现略优于组注意力（90.37% vs 88.84%），但在所有其他情况下相比任何基于RITA的方法都最差。同时，在全部6个多变量数据集中，组注意力始终比Performer和Linformer更快，因此实现双赢。在分类准确率方面，组注意力显著优于PatchTST和Triformer。这是因为这两种方法专门针对时间序列预测设计，其特殊设计方案产生的特征嵌入可能对其他时间序列分析任务产生负面影响（如相关工作部分所述）。此外，在长序列上，组注意力比PatchTST/Triformer快1.3/4.4倍，例如在ECG数据集上，因为处理长序列时组注意力具有更多共享机会。

7.2.2 预训练+少量标签微调（多变量分类）
表2所示的结果得出以下观察：
(1) 预训练是有效的。预训练始终比从头开始使用少量标签进行训练获得更好的准确率。特别是，在WISDM数据上，所有采用RITA架构的方法的准确率至少提高了10%。考虑到我们没有非常大的无标签预训练集可用，这一表现令人印象深刻。
(2) 组注意力相对于TST和TS2VEC的优势。在四个分类数据集中，我们的组注意力平均比TST高出15个百分点，并且比TS2VEC高出6个百分点。
(3) 组注意力相较于其他注意力机制的优势。在4个数据集中有3个数据集上，组注意力优于Performer和Linformer。

> Group Attn. is better than Performer and Linformer on 3 out of 4 datasets. When compared to Vanilla, Group Attn. is better on HHAR and ECG, and comparable on the other two, further confirming its high quality on approximation. Further, we notice that Linformer struggles in this setting: in average its accuracy is worse than Vanilla by 8.22% and Group Attn. by 8.01%. This is because the low-rank projection operation introduces extra model parameters, making Linformer more easily overfit, while overfitting is especially harmful when there are only a few labeled training samples. Triformer and PatchTST, the two timeseries forecasting methods have an accuracy much (34%) lower than Group Attn., for a similar reason to what we have discussed in Sec. 7.2.1. 7.3 Imputation
...
points on average, and outperforms TS2VEC by 6 percentage points. (3) Group Attention’s advantage over other attention mechanisms. Group Attn. is better than Performer and Linformer on 3 out of 4 datasets. When compared to Vanilla, Group Attn. is better on HHAR and ECG, and comparable on the other two, further confirming its high quality on approximation. Further, we notice that Linformer struggles in this setting: in average its accuracy is worse than Vanilla by 8.22% and Group Attn. by 8.01%. This is because the low-rank projection operation introduces extra model parameters, making Linformer more easily overfit, while overfitting is especially harmful when there are only a few labeled training samples. Triformer and PatchTST, the two timeseries forecasting methods have an accuracy much (34%) lower than Group Attn., for a similar reason to what we have discussed in Sec. 7.2.1.
...
Similar to classification tasks, the results of imputation tasks (Table 3) show that Group Attn. consistently outperforms the baselines in training time while achieving comparable/better Mean Square Error (MSE). On the large dataset MGH (length = 10,000), TST, Vanilla and PatchTST fail due to out of memory (OOM) errors. Methods using Rצות: Group Attention is All You Need for Timeseries Analytics 62:21 should decrease gradually. We vary 𝑁 for Fixed, 𝐷𝑒𝑐𝑎𝑦_𝑟𝑎𝑡𝑒 for Heuristic, and the error bound threshold 𝜖 used by RITA. From the results in Table 4 we get the following observations:
(1) Adaptive Scheduler is better than the baselines. Training with Adaptive Scheduler achieves better or comparable performance compared to the best performing 𝑁 or 𝐷𝑒𝑐𝑎𝑦_𝑟𝑎𝑡𝑒. For 𝐹𝑖𝑥𝑒𝑑, on the MGH dataset, dynamic scheduler always achieves better accuracy and is much faster compared to fixed 𝑁 . On the ECG dataset, although fixed 𝑁 is slightly better than adaptive scheduler in accuracy when setting the 𝑁 as 512, it runs much slower than adaptive scheduler. For 𝐻𝑒𝑢𝑟𝑖𝑠𝑡𝑖𝑐, we have similar observations: on ECG dataset, dynamic scheduler consistently achieves better accuracy than 𝐻𝑒𝑢𝑟𝑖𝑠𝑡𝑖𝑐. On MGH dataset, dynamic scheduler achieves comparable MSE with less training time. Note that manually finding the best 𝑁 or 𝐷𝑒𝑐 62:21 should decrease gradually. We vary 𝑁 for Fixed, 𝐷𝑒𝑐𝑎𝑦_𝑟𝑎𝑡𝑒 for Heuristic, and the error bound threshold 𝜖 used by RITA. From the results in Table 4 we get the following observations:
(1) Adaptive Scheduler is better than the baselines. Training with Adaptive Scheduler achieves better or comparable performance compared to the best performing 𝑁 or 𝐷𝑒𝑐𝑎𝑦_𝑟𝑎𝑡𝑒. For 𝐹𝑖𝑥𝑒𝑑, on the MGH dataset, dynamic scheduler always achieves better accuracy and is much faster compared to fixed 𝑁 . On the ECG dataset, although fixed 𝑁 is slightly better than adaptive scheduler in accuracy when setting the 𝑁 as 512, it runs much slower than adaptive scheduler. For 𝐻𝑒𝑢𝑟𝑖𝑠𝑡𝑖𝑐, we have similar observations: on ECG dataset, dynamic scheduler consistently achieves better accuracy than 𝐻𝑒𝑢𝑟𝑖𝑠𝑡𝑖𝑐. On MGH dataset, dynamic scheduler achieves comparable MSE with less training time. Note that manually finding the best 𝑁 or 𝐷𝑒𝑐𝑎𝑦_𝑟𝑎𝑡𝑒 that balances the accuracy and running time requires careful tuning, while our adaptive scheduler does not need any tuning.
(2) Adaptive Scheduler is tuning free. It is robust on both accuracy and running time when 𝜖 varies, while the results of fixed 𝑁 vary significantly when the value of 𝑁 changes. Therefore, Adaptive Scheduler frees the users from tuning the 𝜖 threshold, while it is hard to manually find an appropriate 𝑁 or 𝐷𝑒𝑐𝑎𝑦_𝑟𝑎𝑡𝑒 for a given dataset.
...
> on WISDM dataset with 20%/40%/60%/80% of the pretraining

RITA Pretraining: increasing sizes of pretrain set.
7.5.2 The Sizes of the Pretraining Data
Next, we evaluate how the number of unlabeled data influences the effectiveness of pretraining. To get empirical results, we pretrain RITA on WISDM dataset with 20%/40%/60%/80% of the pretraining
...
Such sparsity is intuitive in language data, in which a word’s attention mainly focuses on the nearby sentences. However, attention in timeseries data shows strong seasonal patterns rather than sparse patterns, mainly as result of the periodicity of timeseries data. Therefore, such works do not work well for timeseries. Apart from introducing attention patterns, some works seek to solve this problem with applied mathematics techniques. Linformer [62] performs a projection to decrease the size of query, key and value matrices before attention computation, because the attention matrix tends to be low-ranked. Performer [11] uses linear functions to approximate the kernel function softmax, making attention computation commutative. Linformer and Performer do not depend on the unique properties of language data, thus potentially fitting times

然而，固定注意力模式严重依赖用户给出最优设置。Reformer[31]提出仅基于其对语言/图像数据中注意力矩阵稀疏性的观察来计算主导注意力项。这种稀疏性在语言数据中是直观的，在其中单词的关注主要集中在邻近句子上。但是时序数据中的注意力表现出强烈的季节模式而非稀疏模式，这主要是由于时序数据的周期性所致。因此，这些方法对时序数据效果不佳。除了引入注意力模式外，一些研究尝试通过应用数学技术解决此问题。Linformer[62]在进行注意力计算前执行投影以减小查询、键和值矩阵的规模，因为注意力矩阵往往具有低秩特性。Performer[11]使用线性函数近似核函数softmax，使注意力计算具备交换性。Linformer 和 Performer 不依赖语言数据的独特属性，因此可能比其他技术更适合时序数据，这也是我们在实验中将其作为对比对象的原因。然而如第7节所示，我们的组注意力在准确性和效率（训练时间）方面均显著优于它们，因为组注意力充分利用了时序数据的周期性。表6总结了针对扩展Transformer以处理长序列的工作的时间/空间复杂度。注意尽管几种方法声称具有线性时间和空间复杂度，但其复杂度都涉及特定方法常数，类似于我们组注意力中的分组数量𝑁。在长期时间序列场景中，当𝑑 ∈ [64,128,256] 且𝑛 ≫ 𝑑 ≈（常见设置）时，其中𝑑表示特征嵌入的维度而𝑛代表时序长度，PatchTST、Triformer、Linformer 和 Performer 的理论复杂度与组注意力处于相同量级。然而我们的实验结果表明，在效率方面组注意力始终显著优于这些方法，如图4、图5和表3所示。这是因为组注意力非常适合处理长时间序列数据。时序越长，组注意力就越有机会找到相似片段并将其分组在一起。这导致了分组数量'𝑁'的增长率变缓——即组注意力的时间/空间复杂度中的常数项。
...
9 结论
在本工作中，我们提出了RITA，一种自动、自监督且可扩展的时序嵌入工具。RITA有效将Transformer（该方法在自然语言处理中流行）适应于将时间序列片段嵌入特征向量。作为RITA的关键组件，组注意力消除了经典自注意力机制的性能瓶颈，从而成功地使RITA适用于高度复杂、长周期的时间序列数据。我们的实验结果证实了RITA通过63倍的速度提升显著加快现有注意力机制，并且具有更高的准确性。

ACKNOWLEDGMENTS
This work was supported in part by NSF grants III-1910108 and DBI-2327954.

致谢
本工作部分得到了NSF资助项目III-1910108和DBI-2327954的支持。
