...
You are the engineering agent for Scholar Assistant, a privacy-first academic writing workspace. Treat the product as **"Codex for papers"**: the agent should work with the user's selected workspace files directly, while protecting drafts, PDFs, bibliographies, data, and export artifacts from accidental overwrite or boundary escape.

Priority rules:
1. Keep user files safe: respect `workspace_root`, never silently access out-of-workspace paths, and never overwrite unsaved Monaco tabs.
2. Preserve cross-layer contracts: SSE event names, `tool_name` metadata, Agent approval payloads, and ledger `?doc_id=` routes must stay synchronized between backend and frontend.
3. Route simple document Q&A away from ReAct: pure questions about the open document use oneshot doc QA; explicit file edits or command execution use the full Agent loop.
4. Prefer existing architecture and tests over new abstractions. Small localized fixes get focused tests; protocol or tool changes need frontend/backend integration coverage.

## Fast Decision Guide

| User intent | Default implementation path |
|-------------|-----------------------------|
| Upload or translate a PDF | `src/composables/useTranslate.ts` <-> `python/routers/translate.py`; keep `translate.*` SSE names exact |
| Ask about an open document without asking for edits | `python/routers/agent.py._oneshot_doc_qa_stream`; do not call tools or write files |
| Modify files or run commands | Agent ReAct loop with workspace boundary approval |
| Use AI panel preset polish/expand/review/translate | `/api/edit` one-shot SSE, not `/api/agent/v2/chat` |
| Build/rebuild argument ledger or reviewer state | `python/routers/argument.py`; ledger routes use `?doc_id=` query params |
| Change SSE payloads or tool metadata | Update backend models, frontend consumers, and tests together |
...
# Backend protocol/tool/session changes
pytest tests/ -v
```
...
"Codex for Papers" 改造 (2026-05-22–23，5刀计划，main 分支)：
...
- 验证：1752 pytest passed / 11 skipped；326 vitest passed；E2E SSE 测试：越界路径 → await_approval → approve → ContextVar bypass → tool_result 全链路通过；计划详情见 `docs/Codex-pivot-todo.md`
...
- `ledger.py` — **实际补上 anchor SSE 事件**（此前 AGENTS.md 声称已做但代码并未实现）：`build_ledger` 在 src_anchor + 每个 discharge anchor 处 yield `anchor` 事件；`rebuild_ledger` 透传 `anchor`。修复前端 `ledger.anchors` 恒空导致"跳转原文/兑付处"按钮全失效
